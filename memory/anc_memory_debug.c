...
static AncMemHashMap *AncMemCreatHashMap() {
    ...
    AncMemHashMap *p_mem_hashmap = (AncMemHashMap *)AncPlatformMalloc(sizeof(AncMemHashMap));
    ...
    p_mem_hashmap->p_mem_table =
        (AncMemHashNode *)AncPlatformMalloc(p_mem_hashmap->mem_table_size * sizeof(AncMemHashNode));
    ...
    for (i = 0; i < p_mem_hashmap->mem_table_size; i++) {
        AncPlatformMemset(&(p_mem_hashmap->p_mem_table[i].mem_block), 0, sizeof(AncMemHashNode));
...
static ANC_RETURN_TYPE AncHashMapPut(AncMemHashMap *p_hashmap, AncMemBlock *p_mem_block) {
    if ((NULL == p_hashmap) || (NULL == p_mem_block)) {
        ANC_LOGE("memory map or put block is null");
        return ANC_FAIL;
    }
    uint32_t index = BKDRHash(p_mem_block->function_name) % (p_hashmap->mem_table_size);
    ...
    if (p_curr_list->mem_block.func_malloc_cnt == 0) {  // no conflict
        ...
        while (p_curr_list->p_next != NULL) {
            if (strcmp(p_curr_list->mem_block.function_name, p_mem_block->function_name) == 0) {
                p_curr_list->mem_block.func_malloc_cnt++;
                break;
            }
...
static ANC_RETURN_TYPE AncMemListInsert(AncMemListRecord **p_list, AncMemBlock *p_mem_info) {
    // todo realloc
    AncMemListRecord *p_record = (AncMemListRecord *)AncPlatformMalloc(sizeof(AncMemListRecord));
    if (NULL == p_record) {
...
void AncRegisterAllocation(void *p_addr, size_t mem_size, const char *p_func_name, int code_line) {
    if (NULL == gp_anc_mem_debug) {
    ...
    if (gp_anc_mem_debug != NULL) {
        ...
        AncPlatformMemcpy(p_mem_info->function_name,
                          p_func_name,
...
static AncMemListRecord *AncListFindPreNode(void *p_addr, AncMemListRecord *p_list) {
    AncMemListRecord *p = p_list;
    while ((p != NULL) && (p->p_next != NULL) && (p->p_next->mem_block.p_mem_addr != p_addr)) {
        p = p->p_next;
    }
    return p;
}
...
void AncUnRegisterAllocation(void *p_addr) {
    ...
    if (p_addr == p_record_tmp->mem_block.p_mem_addr) {
        ...
        AncMemListRecord *p = AncListFindPreNode(p_addr, gp_anc_mem_debug->p_mem_record);
...
static void AncMemoryBlockPrint() {
    ...
    if ((gp_anc_mem_debug != NULL) && (gp_anc_mem_debug->p_mem_record != NULL)) {
        ...
        while (p != NULL) {
            ANC_LOGD("p_addr = 0x%x, mem_size = %d, function name: %s, line = %d",
                     p->mem_block.p_mem_addr,
                     p->mem_block.mem_size,
                     p->mem_block.function_name,
                     p->mem_block.file_line_num);
            p = p->p_next;
        }
    }
